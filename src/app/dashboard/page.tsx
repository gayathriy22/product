...
function Page() {
  ...
  const moveTasktoComplete = async (task) => {
    // Update the task status to complete: true
    const updatedTaskData = JSON.stringify({
      ...task,
      complete: true,
    });
  ...
  const renderTaskCard = () => {
    if (activeTab === '1') {
      const todayTasks = tasks.filter(task => moment(task.date).isSame(moment(), 'day') && !task.complete);
            ...
            <Box style={{ boxShadow: '0px 0px 12px rgba(0, 0, 0, 0.25)', backgroundColor: '#434343', padding: '32px', border: '3px solid #0788FF', borderRadius: '20px', width: '296px', height: '432px', display: 'flex', flexDirection: 'column', alignItems: 'center', marginBottom: '12px', position: 'relative',}}>
              {tasksForPage.map((task, index) => (
                <Card key={task.task_id} style={{ backgroundColor: '#434343', borderColor: '#434343', borderRadius: '20px', width: '286px', height: '426px', position: 'absolute', top: 0, left: 0, display: 'inline-block', flexDirection: 'column', alignItems: 'center'}}>
                  ...
                  <Button type="primary" onClick={() => moveTasktoComplete(task)} style={{ width: '224px', height: '40px', padding: '10px 76px', color: '#000', borderRadius: '14px', fontWeight: 'bold', fontSize: '14px', background: '#B1DFFF', display: 'flex', justifyContent: 'center', alignItems: 'center', marginTop: '200px', marginBottom: '12px', fontFamily: 'Poppins'}}>Complete</Button>
                </Card>
              ))}
            </Box>
      ...
      const laterTasks = tasks.filter(task => !moment(task.date).isSame(moment(), 'day')  && !task.complete);
